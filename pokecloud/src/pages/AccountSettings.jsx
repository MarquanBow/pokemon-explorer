...
function Message({ msg }) {
  ...
  return (
    <p className={`text-sm font-semibold px-4 py-2.5 rounded-xl mt-3 ${
      msg.type === "success"
...
export default function AccountSettings() {
  ...
  // Redirect if not logged in; seed form from context user
  useEffect(() => {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      refreshUser(); // push updated displayName/photoURL to every component
        ...
        <div className="bg-white rounded-3xl shadow-md border border-gray-100 p-6 mb-6 flex items-center gap-5">
          ...
          <div>
            <p className="font-black text-gray-800 text-xl">
              {user?.displayName || <span className="text-gray-400 italic font-semibold">No name set</span>}
            </p>
